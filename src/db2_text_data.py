...
class StreamingTextDataset(StreamingDataset):
    ...
    def _tokenize(self, text_sample):
        if self.tokenizer.pad_token is None: #if self.tokenizer._pad_token is None:
    ...
    def __getitem__(self, idx: int) -> Union[Dict[str, Any], torch.Tensor]:
        ...
        if 'text' in sample:
            seq = sample['text']
            n_count = seq.upper().count('N') # Filter sequences with >25% N content #NOTE-ADDED THIS BLOCK LATER AFTER THE TOKEN CHECK
            if n_count / max(len(seq), 1) > 0.25:
                # Return a valid non-N sequence by wrapping to next index
                # Simple approach: return a fixed fallback or skip via replacement
                # For MDS datasets, replace with a nearby sample
                replacement_idx = (idx + 1) % len(self)
                sample = super().__getitem__(replacement_idx)
