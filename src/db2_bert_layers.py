...
from db2_bert_padding import (index_first_axis, index_put_first_axis, pad_input, unpad_input, unpad_input_only)
import os

# --- FA2 import (preferred - CUDA kernels with native alibi_slopes) ---
try:
    from flash_attn import flash_attn_varlen_qkvpacked_func
    _FA2_AVAILABLE = True
    print("n2_bert_layers.py: flash Attention 2 loaded successfully (alibi_slopes supported)")
except ImportError:
    flash_attn_varlen_qkvpacked_func = None
    _FA2_AVAILABLE = False

# --- FA4 import (CuTeDSL — Blackwell/Hopper, no native ALiBi) ---
try:
    from flash_attn.cute import flash_attn_varlen_func as flash_attn_varlen_func_fa4
    _FA4_AVAILABLE = True
    if not _FA2_AVAILABLE:
        print("[INFO] Flash Attention 4 (CuTeDSL) loaded (no alibi_slopes — will use SDPA for ALiBi)")
except ImportError:
    flash_attn_varlen_func_fa4 = None
    _FA4_AVAILABLE = False

if not _FA2_AVAILABLE and not _FA4_AVAILABLE:
    print("[INFO] No Flash Attention backend found — using PyTorch SDPA")
...
class BertUnpadSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Check FA4/FA2 availability. Check attention backend availability
        if not _FA2_AVAILABLE and not _FA4_AVAILABLE:
            warnings.warn(
                'Not using FA2 or FA4; using PyTorch SDPA for attention '
                '(still performant via cuDNN backend on B200) Change the If-Else clause to use.'
            )
    ...
    def forward(self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor,
        ...
        if hasattr(torch, 'float8_e4m3fn'):
            supported_dtypes.extend([torch.float8_e4m3fn, torch.float8_e5m2])

        if _FA2_AVAILABLE:
            # ----- FA2 path: packed QKV + native alibi_slopes -----
            qkv = rearrange(qkv, 'n (t h d) -> n t h d', t=3, h=self.num_attention_heads)

            # Ensure bf16/fp16/fp8 for FA2
            ...
            if convert_dtype:
                attention = attention.to(hidden_states.dtype) #attention = attention.to(torch.bfloat16)

            return rearrange(attention, 'n h d -> n (h d)')

        else:
            # ----- PyTorch SDPA fallback: explicit ALiBi bias matrix -----
            qkv = pad_input(qkv, indices, cu_seqlens.shape[0] - 1,
                        max_seqlen_in_batch)  # (B, S, 3*H*D)
            ...
            # Build ALiBi attention bias: (1, H, S, S)
            sdpa_attn_mask = None
            if bias is not None and bias.dim() == 1:
                seqlen = max_seqlen_in_batch
                positions = torch.arange(seqlen, device=bias.device,
                                     dtype=torch.float32)
                rel_pos = torch.abs(positions.unsqueeze(0) -
                                positions.unsqueeze(1))  # (S, S)
                alibi_bias = -(bias.float().view(1, -1, 1, 1) *
                            rel_pos.view(1, 1, seqlen, seqlen))
                sdpa_attn_mask = alibi_bias.to(q.dtype)

            # Incorporate padding mask
            if attn_mask is not None:
                pad_mask = attn_mask.bool()  # (B, S)
                pad_bias = torch.zeros(
                    pad_mask.shape[0], 1, 1, pad_mask.shape[1],
                    device=q.device, dtype=q.dtype)
                pad_bias.masked_fill_(
                    ~pad_mask.unsqueeze(1).unsqueeze(2), float('-inf'))
                if sdpa_attn_mask is not None:
                    sdpa_attn_mask = sdpa_attn_mask + pad_bias
                else:
                    sdpa_attn_mask = pad_bias
            ...
            # Un-pad: (B, S, H, D) → (total_nnz, H, D)
            attention = unpad_input_only(attention,
                                     torch.squeeze(attn_mask) == 1)
            attention = rearrange(attention, 'n h d -> n (h d)')
            return attention.to(hidden_states.dtype)
# Note - FA4 is still in Beta at the time of development, hence the attention code note added 
...
class BertEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        #  self.alibi is (H,) -> O(1) memory  (e.g. 48 bytes)
    ...
    def rebuild_alibi_tensor(self,
        ...
        #   Triton: Builds full (1, H, N, N) relative position matrix -> O(N^2)
        #   FA2: Only stores (H,) slopes -> FA2 kernel computes bias on-the-fly
    ...
    def forward(
        ...
        # [FA2 ACTIVE] Block 5: Pass slopes directly instead of O(N^2) bias.
        #   Triton: alibi_attn_mask is (B,H,N,N) = O(B*H*(N^2)) memory
        #   FA2: alibi_attn_mask is (H,) slopes = O(1) memory
