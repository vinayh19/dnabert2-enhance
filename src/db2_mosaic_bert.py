...
import db2_bert_layers as bert_layers_module
import db2_configuration_bert as configuration_bert_module
import transformers
...
from db2_genomic_metrics import MaskedPerplexity
...
def create_mosaic_bert_mlm(pretrained_model_name: str = 'bert-base-uncased',
    ...
    metrics = [LanguageCrossEntropy(ignore_index=-100, ), 
...
def create_mosaic_bert_classification(
    ...
    # Use `alibi_starting_size` to determine how large of an alibi tensor to create when initializing the model. You should be able to ignore this parameter in most cases.
    ...
    hf_model = HuggingFaceModel(model=model, tokenizer=tokenizer,
                                use_logits=True, metrics=metrics)
    return hf_model
